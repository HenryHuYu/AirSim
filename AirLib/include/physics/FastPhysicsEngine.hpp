...
namespace airlib
{
    ...
    private:
        ...
        void updatePhysics(PhysicsBody& body)
        {
            ...
            getNextKinematicsNoCollisionSubstepped(dt, body, current, next, next_wrench, wind_);
        ...
        */
        ...
        static void getNextKinematicsNoCollisionSubstepped(TTimeDelta dt, PhysicsBody& body, const Kinematics::State& current,
                                                           Kinematics::State& next, Wrench& next_wrench, const Vector3r& wind)
        {
            if (dt <= kMaxPhysicsTimeStep) {
                getNextKinematicsNoCollision(dt, body, current, next, next_wrench, wind);
                return;
            }

            const uint substep_count = static_cast<uint>(std::ceil(dt / kMaxPhysicsTimeStep));
            const TTimeDelta substep_dt = dt / substep_count;
            Kinematics::State substep_current = current;
            Wrench substep_wrench = Wrench::zero();

            // Small custom frames have low inertia and high acceleration, so keep the
            // integrator step bounded even when ClockSpeed increases the outer dt.
            for (uint substep = 0; substep < substep_count; ++substep) {
                getNextKinematicsNoCollision(substep_dt, body, substep_current, next, substep_wrench, wind);
                substep_current = next;
            }

            next_wrench = substep_wrench;
        }
    ...
    private:
        ...
        static constexpr float kMaxPhysicsTimeStep = 0.0006f;
